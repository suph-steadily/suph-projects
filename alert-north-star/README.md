...
- **Where the 115 comes from, and the bracket above it.** ~6,000 age-alert-only quotes a month bind at ~15.7% today. The sister cohort binds at ~18%, and that is also what clean under-91 homes with zero alerts bind at (18.0%), so 18% is the standard no-alert rate for old rental homes: 15.7% to 18% on ~6,000 quotes = the +115 floor. The ceiling: the never-flagged book overall binds at 25.4%; taking old homes all the way there would mean ~+575 a month, but that book is newer, cleaner property, so treat it as a ceiling, not a projection. Say it as: **between +115 and roughly +575 binds a month, and we defend the 115.** Anything above the floor is the alert's scare-off effect coming back.
